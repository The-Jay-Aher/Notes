...
The local prompt file named `_System/Instructions/master-subject-prompts.md` is not a Docker-only request. It is a master request for multiple learning tracks. This file records the authoritative scope, current repository coverage, source assumptions, and the rollout plan so the vault is upgraded without duplicating existing notes or losing the structure requested in `AGENTS.md`.

The goal is not to create shallow folders. The goal is to create or upgrade mastery-level notes for every track named in `_System/Instructions/master-subject-prompts.md`, using source-backed material and the repository's chapter standard.

## Current Scope From `_System/Instructions/master-subject-prompts.md`

| Track | Required action | Current vault state | Initial status |
| --- | --- | --- | --- |
| CKA | Upgrade existing notes | `notes/devops-cloud/cka/` upgraded with labs/revision files | Current exam snapshot refreshed; core command labs, cheatsheet, glossary, changelog added |
| CKAD | Upgrade existing notes | `notes/devops-cloud/ckad/` upgraded with labs/revision files | Current exam snapshot refreshed; core application labs, cheatsheet, glossary, changelog added |
| Kubernetes Architecture | Create or upgrade | `notes/devops-cloud/kubernetes/` core architecture chapters drafted | Core architecture flows complete; future provider-specific labs can deepen coverage |
| AWS Solutions Architect Associate | Upgrade existing notes | `notes/devops-cloud/aws-solutions-architect-associate/` upgraded with SAA-C03 alignment and revision files | Core upgraded; future full mock-exam sets can deepen coverage |
| AWS Solutions Architect Professional | Create or upgrade | `notes/devops-cloud/aws-solutions-architect-professional/` core chapters drafted | Core drafted; future scenario-question sets can deepen coverage |
| Docker | Upgrade existing notes | `notes/devops-cloud/docker/` completed and pushed in commit `28495ea` | Completed for Docker scope |
| Ansible | Upgrade existing notes | `notes/devops-cloud/ansible/` upgraded with idempotency labs/revision files | Core upgraded; future real-infra role examples can deepen coverage |
| Jenkins | Upgrade existing notes | `notes/devops-cloud/jenkins/` upgraded with operations/security/revision files | Core upgraded; future controller-as-code and Kubernetes-agent labs can deepen coverage |
| ArgoCD | Create or upgrade | `notes/devops-cloud/argocd/` core chapters drafted | Core drafted; future labs can deepen Helm/Kustomize and secret-tool specifics |
| Competitive Programming | Create or upgrade | `notes/programming/competitive-programming/` core chapters drafted | Core drafted; future difficulty-graded problem sets can deepen coverage |
| Java | Create or upgrade | `notes/programming/java/` core chapters drafted | Core drafted; future labs and framework examples can deepen coverage |
| Python | Create or upgrade | `notes/programming/python/` core chapters drafted | Core drafted; future labs and larger interview sets can deepen coverage |
| C++ | Create or upgrade | `notes/programming/cpp/` core chapters drafted | Core drafted; future advanced template/concurrency labs can deepen coverage |
| UPSC | Upgrade existing notes | `notes/competitive-exams/upsc/Notes/` upgraded with 2026 source-refresh and answer-writing playbook | Core upgraded; future PYQ solved sets can deepen coverage |
| MPSC | Create or upgrade | `notes/competitive-exams/mpsc/` core chapters drafted | Core drafted; official pattern/data must be verified from latest MPSC PDFs |
| RBI Grade B | Create or upgrade | `notes/competitive-exams/rbi-grade-b/` core chapters drafted with official-verification warning | Core drafted; current notification facts require manual RBI PDF verification |
| RBI Assistant | Create or upgrade | `notes/competitive-exams/rbi-assistant/` core chapters drafted with official-verification warning | Core drafted; current notification facts require manual RBI PDF verification |
| SEBI Grade A | Create or upgrade | `notes/competitive-exams/sebi-grade-a/` core chapters drafted from accessible official PDFs | Core drafted; future stream-specific deep sets can deepen coverage |
...
5. Maintain this file as the audit map until every topic from `_System/Instructions/master-subject-prompts.md` is proven covered.

## Completion Criteria

The full goal is not complete until all tracks named in `_System/Instructions/master-subject-prompts.md` have:

- no duplicate conflicting folder
- existing notes upgraded or missing notes created
- source-backed roadmap
- required topic coverage from `_System/Instructions/master-subject-prompts.md`
- causal chains
- diagrams where useful
...
- cheatsheet or revision sheet
- changelog
- final coverage audit showing no topic from `_System/Instructions/master-subject-prompts.md` is missing
...
Result: all tracks named in `_System/Instructions/master-subject-prompts.md` now have repository coverage that satisfies the core `AGENTS.md` requirements for this rollout.
...
| Required topic coverage from `_System/Instructions/master-subject-prompts.md` | Passed at core-note level across CKA, CKAD, Kubernetes Architecture, AWS SAA, AWS SAP, Docker, Ansible, Jenkins, ArgoCD, Competitive Programming, Java, Python, C++, UPSC, MPSC, RBI Grade B, RBI Assistant, and SEBI Grade A. |
